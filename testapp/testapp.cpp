...
public:
	...
	ProvidersList GetProvidersList()
	{
		...
		for (int j = 0; j < lpl2->dwNumProviders; j++)
		{
			...
			pd.providerName = std::string((char*)(v.data() + lpeptr[j].dwProviderFilenameOffset), lpeptr[j].dwProviderFilenameSize - 1);
			retVal.push_back(pd);
		}
		return retVal;
	}
...
void TestConfigProv(int ac, char *av[])
{
	printf("TestConfigProv\n");
	if (ac < 3)
	{
		printf("usage: testapp.exe configprov <device id>\n");
		return;
	}
	std::string idStr = av[2];
	std::stringstream s(idStr);
	int providerId;
	s >> providerId;
	LONG n = lineConfigProvider(NULL, providerId);
	printf("lineConfigProvider returned: %08x\n", n);
}

void TestConfigProvByName(int ac, char *av[])
{
	printf("TestConfigProvByName\n");
	if (ac < 3)
	{
		printf("usage: testapp.exe TestConfigProvByName <provider name>\n");
		return;
	}
	std::string provName = av[2];
	printf("provName = %s\n", provName.c_str());
	int providerId;
	DialTester dt;
	DialTester::ProvidersList p = dt.GetProvidersList();
	for (int j = 0; j < p.size(); j++)
	{
		auto n = p[j].providerName.rfind(provName);
		if (n != -1)
		{
			int l1 = n + provName.size();
			int l2 = p[j].providerName.size();
			if (l1 == l2)
			{
				providerId = p[j].providerId;
				break;
			}
		}
	}
...
int main(int ac, char *av[])
{
	...
	if (ac < 2)
	{
		...
		printf("commands: install, uninstall, dial, answer, getprovlist, getlineslist, configprov, configprovbyname\n");
		return 0;
	}
	...
	else if (command == "configprov")
		TestConfigProv(ac, av);
	else if (command == "configprovbyname")
		TestConfigProvByName(ac, av);
	else
	{
